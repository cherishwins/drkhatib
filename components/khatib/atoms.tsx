...
export function GoldRule({
  ...
  const widthClass = width === 'full' ? 'w-full' : width === 'short' ? 'w-16' : '';
...
export function Eyebrow({
  ...
  const colorClass =
    tone === 'gold' ? 'text-gold' : tone === 'navy' ? 'text-deep-navy' : 'text-warm-gray';
  return (
    <div
      className={clsx(
        'flex items-center gap-3 font-mono text-[11px] uppercase tracking-tracked-wide',
        colorClass,
        className,
      )}
    >
      {withRule && (
        <GoldRule width="short" className={tone === 'navy' ? 'bg-deep-navy' : undefined} />
      )}
      <span>{children}</span>
    </div>
  );
